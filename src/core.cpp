...
Game::Game() {
	...
	window = SDL_CreateWindow(TITLE, SDL_WINDOWPOS_CENTERED,
			SDL_WINDOWPOS_CENTERED, DISPLAY_WIDTH, DISPLAY_HEIGHT,
			SDL_WINDOW_SHOWN);
...
void Game::loop() {
	...
	while (true) {
		...
		dt_s = (dt_current_time - dt_last_time)
		...
		updating_time_ms = 1000.0f * (update_end_time - update_start_time)
				/ (float) SDL_GetPerformanceFrequency();

		render_start_time = SDL_GetPerformanceCounter();
		render();
		render_endtime = SDL_GetPerformanceCounter();
		rendering_time_ms = 1000.0f * (render_endtime - render_start_time)
				/ (float) SDL_GetPerformanceFrequency();

		fps_end_time = SDL_GetPerformanceCounter();
		// printf("dt: %f s, FPS: %f Hz,\n", dt_s, 1.0f/dt_s);
		fps_time_ms = 1000.0f * (render_endtime - render_start_time)
				/ (float) SDL_GetPerformanceFrequency();
...
//				board->place_random_piece();
	...
	board_renderer->process_event_queue();
